...
def create_user():
    ...
    if "user_image" in request.files and request.files['user_image'].filename != '':
        ...
        if not validate_type_of_uploaded_media_file(media_type, { 'image': ('apng', 'bmp', 'gif', 'jpeg', 'jpg', 'png', 'webp', 'svg') }):
            flash("An uploaded file type is not image!", 'user_image')
            error = True
...
# This function will be excuted before each action, movement and each route excution; to validate the identity of the user.
...
def replace_job_title(required_input):
    job_titles = {
        1: "Adminstrator",
        2: "Manager",
        3: "Worker"
    ...
    if type(required_input) == list:
        for member in required_input:
            member['job_title'] = job_titles[member['job_title']]
        return required_input
    return job_titles[required_input]
...
def delete_user(user_id_url):
    ...
    if checked_user['user_job_title'] == "Adminstrator" and int(user_id_url) == 1:
        return redirect("/sign_out")
    ...
    # Only the admin can delete any account & the user can delete his own account
        ...
        if checked_user['user_job_title'] == "Adminstrator":
...
# Route to get all managers data.
...
# Route to get all new_workers data (with no team/supervisor yet).
...
def render_my_team():
    ...
    if checked_user:
        ...
        return render_template("users/my_team.html", has_team = my_team_data['has_team'], the_supervisor = my_team_data['the_supervisor'], the_team = my_team_data['the_team'], checked_user=checked_user)
    return redirect("/sign_out")


# Get data of my_team.
@app.route("/teams/get_my_team")
def get_my_team():
    checked_user = check_user_id()
    ...
    if checked_user:
        if checked_user['user_job_title'] == "Worker":
            ...
            if len(supervisor)>0:
                ...
                return jsonify(has_team=True, the_supervisor=supervisor, the_team=team)
            else:
                return jsonify (has_team=False, the_supervisor=None, the_team=None)
